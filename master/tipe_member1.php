...
if (isset($_GET['toggle_id'])) {
    ...
    $stmt_check = safe_sqlsrv_query($conn, "SELECT Status, Nama_Tipe FROM Tipe_Member WHERE ID_Tipe = ?", array($toggle_id), false);
    if ($stmt_check !== false) {
        ...
        $stmt_toggle = safe_sqlsrv_query($conn, 
            "UPDATE Tipe_Member SET Status = ?, Modified_By = ?, Modified_Date = GETDATE() WHERE ID_Tipe = ?", 
            array($new_status, $modified_by, $toggle_id), false
        );

        if ($stmt_toggle) {
            $status_text = $new_status == 1 ? 'Aktif' : 'Nonaktif';
            header("Location: tipe_member.php?page=1&status=success&msg=Status tipe member berhasil diubah menjadi " . $status_text . "!");
        } else {
            header("Location: tipe_member.php?page=1&status=error&msg=Gagal mengubah status tipe member!");
        }
        exit();
    }
}
...
if (isset($_GET['detail_id'])) {
    $r_detail = safe_sqlsrv_query($conn, "SELECT ID_Tipe, Nama_Tipe, Harga_Member, Potongan_Harga, Status FROM Tipe_Member WHERE ID_Tipe = ?", array($_GET['detail_id']), false);
    if ($r_detail) {
...
$sort_by = isset($_GET['sort']) ? $_GET['sort'] : 'ID_Tipe';
...
$allowed_sort = ['ID_Tipe', 'Nama_Tipe', 'Harga_Member', 'Potongan_Harga', 'Created_Date'];
if (!in_array($sort_by, $allowed_sort)) $sort_by = 'ID_Tipe';
...
// --- STAT COUNTS ---
$count_sql = "SELECT COUNT(*) as t FROM Tipe_Member WHERE " . $where_sql;
$q_total = safe_sqlsrv_query($conn, $count_sql, $params, false);
$total_tipe = 0;
if ($q_total !== false) {
    $row_total = safe_sqlsrv_fetch_array($q_total, SQLSRV_FETCH_ASSOC);
    $total_tipe = $row_total['t'] ?? 0;
}

// Total Aktif
$q_aktif = safe_sqlsrv_query($conn, "SELECT COUNT(*) as t FROM Tipe_Member WHERE Status = 1", [], false);
$total_aktif = 0;
if ($q_aktif !== false) {
...
// Total Nonaktif
$q_nonaktif = safe_sqlsrv_query($conn, "SELECT COUNT(*) as t FROM Tipe_Member WHERE Status = 0", [], false);
$total_nonaktif = 0;
if ($q_nonaktif !== false) {
...
$total_pages = max(1, ceil($total_tipe / $limit));
...
$query_sql = "SELECT ID_Tipe, Nama_Tipe, Harga_Member, Potongan_Harga, Status FROM Tipe_Member WHERE " . $where_sql . " ORDER BY " . $sort_by . " " . $sort_order . " OFFSET " . intval($offset) . " ROWS FETCH NEXT " . intval($limit) . " ROWS ONLY";
...
.info-val.potongan-val { color: var(--orange); font-weight: 800; }
...
/* ============================================
   FILTER - SAMA PERSIS KAYA CUSTOMER
   ============================================ */
...
.tipe-id { font-family: 'Barlow Condensed', sans-serif; font-weight: 800; color: var(--orange); font-size: 14px; }
</style>
</head>
<body>
<aside class="sidebar">
    ...
    <div class="content">
        <div class="page-header">
            ...
            <div class="stat-chips">
                ...
                <div class="stat-chip chip-blue"><i class="fa-solid fa-crown"></i> TOTAL <span class="chip-val"><?= $total_tipe ?></span></div>
            </div>
        </div>
                        ...
                        <form method="GET" id="filterForm">
                            ...
                            <div class="filter-group">
                                ...
                                <select name="sort" class="filter-select">
                                    <option value="ID_Tipe" <?= $sort_by == 'ID_Tipe' ? 'selected' : '' ?>>ID Tipe Member</option>
                                    <option value="Nama_Tipe" <?= $sort_by == 'Nama_Tipe' ? 'selected' : '' ?>>Nama Tipe</option>
                                    <option value="Harga_Member" <?= $sort_by == 'Harga_Member' ? 'selected' : '' ?>>Harga Member</option>
                                    <option value="Potongan_Harga" <?= $sort_by == 'Potongan_Harga' ? 'selected' : '' ?>>Potongan Harga</option>
                            ...
                            <div class="filter-actions">
                                <a href="tipe_member.php" class="btn-filter-reset">
                ...
                <table class="data-table" id="tbl">
                    <thead>
                        <tr>
                            ...
                            <th>Nama Tipe</th>
                            <th style="text-align: left; width: 22%;">Harga Member</th>
                            <th style="text-align: left; width: 22%;">Potongan Harga</th>
                    ...
                    ?>
                        <tr>
                            ...
                            <td class="tipe-name"><?= htmlspecialchars($row['Nama_Tipe']) ?></td>
                            <td class="tipe-harga"><?= format_rupiah($row['Harga_Member']) ?></td>
                            <td class="tipe-potongan"><?= format_rupiah($row['Potongan_Harga']) ?></td>
                            <td>
                            ...
                            <td>
                                <div class="actions">
                                    <a href="?detail_id=<?= htmlspecialchars($row['ID_Tipe']) ?>" class="btn-action btn-view" title="Lihat Detail">
                                        <i class="fa-solid fa-eye"></i>
                                    </a>
                                    <label class="toggle-switch" title="<?= $status_int === 1 ? 'Nonaktifkan' : 'Aktifkan' ?>">
                                        <input type="checkbox" <?= $status_int === 1 ? 'checked' : '' ?> onchange="confirmToggle('<?= $row['ID_Tipe'] ?>', '<?= htmlspecialchars($row['Nama_Tipe']) ?>', <?= $status_int ?>)">
                    ...
                    <?php endwhile; endif; ?>
                    <?php if (!$has_data): ?>
                        <tr><td colspan="6"><div class="empty-state"><i class="fa-solid fa-crown"></i><div>Belum ada data tipe member</div><div style="font-size: 12px; font-weight: 500; margin-top: 8px; opacity: .7;">Data tipe member akan muncul di sini setelah ditambahkan</div></div></td></tr>
                    <?php endif; ?>
                    </tbody>
                </table>
            </div>
        ...
        <div class="pagination-wrap">
            <div class="pagination-info">Menampilkan <strong><?= (($page - 1) * $limit) + 1 ?></strong> - <strong><?= min($page * $limit, $total_tipe) ?></strong> dari <strong><?= $total_tipe ?></strong> data</div>
            <div class="pagination-nav">
                <a href="?page=1&sort=<?= $sort_by ?>&order=<?= $sort_order ?>&status_filter=<?= $filter_status ?>" class="page-btn <?= $page <= 1 ? 'disabled' : '' ?>" title="Halaman Pertama"><i class="fa-solid fa-angles-left"></i></a>
                <a href="?page=<?= $page - 1 ?>&sort=<?= $sort_by ?>&order=<?= $sort_order ?>&status_filter=<?= $filter_status ?>" class="page-btn <?= $page <= 1 ? 'disabled' : '' ?>" title="Halaman Sebelumnya"><i class="fa-solid fa-angle-left"></i></a>
                <?php $start_page = max(1, $page - 2); $end_page = min($total_pages, $page + 2); if ($end_page - $start_page < 4 && $total_pages >= 5) { if ($start_page == 1) { $end_page = min(5, $total_pages); } else { $start_page = max(1, $total_pages - 4); } } if ($start_page > 1): ?><a href="?page=1&sort=<?= $sort_by ?>&order=<?= $sort_order ?>&status_filter=<?= $filter_status ?>" class="page-btn">1</a><?php if ($start_page > 2): ?><span class="page-ellipsis">...</span><?php endif; ?><?php endif; ?>
                <?php for ($i = $start_page; $i <= $end_page; $i++): ?><a href="?page=<?= $i ?>&sort=<?= $sort_by ?>&order=<?= $sort_order ?>&status_filter=<?= $filter_status ?>" class="page-btn <?= $i == $page ? 'active' : '' ?>"><?= $i ?></a><?php endfor; ?>
                <?php if ($end_page < $total_pages): ?><?php if ($end_page < $total_pages - 1): ?><span class="page-ellipsis">...</span><?php endif; ?><a href="?page=<?= $total_pages ?>&sort=<?= $sort_by ?>&order=<?= $sort_order ?>&status_filter=<?= $filter_status ?>" class="page-btn"><?= $total_pages ?></a><?php endif; ?>
                <a href="?page=<?= $page + 1 ?>&sort=<?= $sort_by ?>&order=<?= $sort_order ?>&status_filter=<?= $filter_status ?>" class="page-btn <?= $page >= $total_pages ? 'disabled' : '' ?>" title="Halaman Selanjutnya"><i class="fa-solid fa-angle-right"></i></a>
                <a href="?page=<?= $total_pages ?>&sort=<?= $sort_by ?>&order=<?= $sort_order ?>&status_filter=<?= $filter_status ?>" class="page-btn <?= $page >= $total_pages ? 'disabled' : '' ?>" title="Halaman Terakhir"><i class="fa-solid fa-angles-right"></i></a>
            </div>
        </div>
        <?php else: ?>
        <div class="pagination-wrap"><div class="pagination-info">Menampilkan <strong>1</strong> - <strong><?= $total_tipe ?></strong> dari <strong><?= $total_tipe ?></strong> data</div></div>
        <?php endif; ?>
        <?php endif; ?>
    </div>
</main>
...
<div class="modal-overlay <?= $show_detail ? 'open' : '' ?>" id="modalDetail">
    <div class="modal-box" style="width: 420px; max-height: 85vh; overflow-y: auto;">
        ...
        <div class="modal-header" style="border-bottom: none; padding-bottom: 0; padding-top: 20px;">
            ...
            <div class="modal-title">Detail Tipe Member</div>
        </div>
        <div class="modal-body" style="padding-top: 8px;">
            ...
            ?>
                ...
                <div class="info-row">
                    ...
                    <span class="info-val id-code"><?= htmlspecialchars($detail_data['ID_Tipe']) ?></span>
                </div>
                <div class="info-row">
                    <span class="info-key"><i class="fa-solid fa-crown"></i> Nama Tipe</span>
                    <span class="info-val"><?= htmlspecialchars($detail_data['Nama_Tipe']) ?></span>
                </div>
                <div class="info-row">
                    <span class="info-key"><i class="fa-solid fa-money-bill-wave"></i> Harga Member</span>
                    <span class="info-val harga-val"><?= format_rupiah($detail_data['Harga_Member']) ?></span>
                </div>
                <div class="info-row">
                    <span class="info-key"><i class="fa-solid fa-percent"></i> Potongan Harga</span>
                    <span class="info-val potongan-val"><?= format_rupiah($detail_data['Potongan_Harga']) ?></span>
                </div>
...
function searchTable() {
    ...
    for (var i = 1; i < rows.length; i++) {
        ...
        var tdHarga = rows[i].getElementsByTagName('td')[2];
        if (tdName || tdHarga) {
            var match = false;
            if (tdName && tdName.textContent.toUpperCase().indexOf(input) > -1) match = true;
            if (tdHarga && tdHarga.textContent.toUpperCase().indexOf(input) > -1) match = true;
