                            ...
                            <div class="alat-card-toggle">
                                ...
                                <label class="toggle-switch" title="<?= $is_aktif ? 'Nonaktifkan' : 'Aktifkan' ?>">
                                    <input type="checkbox" <?= $is_aktif ? 'checked' : '' ?>
                                        onchange="confirmToggle('<?= intval($row['ID_Alat']) ?>', '<?= htmlspecialchars($row['Nama_Alat'], ENT_QUOTES) ?>', <?= intval($row['Status']) ?>, this)">
...
<head>
    ...
    <style>
        ...
        *::after {
        ...
        .alat-card:nth-child(odd) {
            background-color: #FFF7ED;
        }

        .alat-card:nth-child(even) {
        ...
        .alat-card:hover .alat-card-photo-wrap img {
        ...
        .alat-card:hover .alat-card-actions {
            transform: scale(1.05);
        }
        ...
        .size-item input::-webkit-outer-spin-button,
        .size-item input::-webkit-inner-spin-button {
            -webkit-appearance: none;
            margin: 0;
        }

        .size-item input::placeholder {
            color: #C4C9D0;
            font-weight: 500;
        }
        ...
        .detail-size-chip:hover {
            border-color: var(--orange);
            background: #fff;
        }

        .detail-size-chip .ds-size {
            font-family: 'Barlow Condensed', sans-serif;
            font-size: 13px;
            font-weight: 800;
            color: var(--muted);
...
<body>
    ...
    <script>
        ...
        function changePage(page) {
            if (page < 1) return; // Cegah angka 0 atau minus

            // Cegah ngeklik halaman yang sama
        ...
        function handlePhotoUpload(input) {
            ...
            if (valPhoto) {
                valPhoto.classList.remove('show');
                valPhoto.innerHTML = '';
            }
        ...
        function renderSizeInputs(kategori, presets) {
            ...
            if (valSizes) {
                valSizes.classList.remove('show');
                valSizes.innerHTML = '';
            }
        ...
        // ============================================
        async function confirmToggle(id, name, currentStatus, checkboxElement) {
            ...
            if (result.isConfirmed) {
                ...
                try {
                    ...
                    if (res.success) {
                        ...
                        checkboxElement.checked = !checkboxElement.checked; // Balikin switch
                        showError('Gagal!', res.msg);
                    }
                } catch (error) {
                    checkboxElement.checked = !checkboxElement.checked; // Balikin switch
                ...
                checkboxElement.checked = !checkboxElement.checked; // Balikin switch kalau dibatalkan
            }
        }
    ...
    document.getElementById('filterCardCustom').classList.remove('open');
        ...
        function validateForm() {
            let valid = true;
            document.querySelectorAll('.modal-input').forEach(el => el.classList.remove('error'));
            document.querySelectorAll('.val-msg').forEach(el => {
