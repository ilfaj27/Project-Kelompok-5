...
<head>
    ...
    <style>
        ...
        .lapangan-card { background: var(--card-bg); border-radius: 16px; border: 1px solid var(--border); overflow: hidden; transition: all .25s ease; cursor: pointer; position: relative; }
        .lapangan-card:hover { transform: translateY(-4px); box-shadow: 0 12px 32px rgba(0, 0, 0, .12); border-color: var(--orange); background-color: #FFEDD5 !important; }
        ...
        .lapangan-card-photo-wrap img { position: absolute; top: 0; left: 0; width: 100%; height: 100%; object-fit: cover; z-index: 1; transition: transform .3s ease; display: block; }
        ...
        .lapangan-card-badge { position: absolute; top: 8px; left: 8px; padding: 7px 16px; border-radius: 20px; font-size: 12px; font-weight: 800; text-transform: uppercase; letter-spacing: .3px; z-index: 2; display: inline-flex; align-items: center; gap: 6px; }
        ...
        .lapangan-card-actions { position: absolute; top: 8px; right: 8px; display: flex; gap: 6px; opacity: 1; z-index: 3; }
        .lapangan-card-action-btn { width: 38px; height: 38px; border-radius: 10px; border: 1.5px solid transparent; display: flex; align-items: center; justify-content: center; cursor: pointer; font-size: 14px; transition: all .25s cubic-bezier(.4, 0, .2, 1); backdrop-filter: blur(4px); text-decoration: none; font-weight: 700; }
        ...
        .modal-input { width: 100%; padding: 12px 14px; border: 1.5px solid var(--border); border-radius: 10px; font-size: 13px; font-family: 'Barlow', sans-serif; margin-bottom: 18px; outline: none; transition: all .2s; color: var(--text); }
        ...
        .btn-submit { width: 100%; background: var(--orange); color: #fff; border: none; padding: 14px; border-radius: 10px; font-weight: 800; font-size: 13px; cursor: pointer; transition: all .2s; text-transform: uppercase; letter-spacing: .5px; display: flex; align-items: center; justify-content: center; gap: 8px; margin-top: 20px; }
        ...
        .val-msg { font-size: 11px; color: var(--red); font-weight: 600; margin-top: -12px; margin-bottom: 18px; display: none; min-height: 16px; }
        ...
        @media(max-width:768px) { .sidebar { width: 0; overflow: hidden; padding: 0; } .main { margin-left: 0; } .content { padding: 20px; } .lapangan-grid { grid-template-columns: repeat(2, 1fr); gap: 12px; } .modal-box { width: 90%; margin: 20px; } .search-box { width: 100%; } .action-bar { flex-direction: column; align-items: stretch; } .detail-photo-wrap { width: 100px; height: 100px; } }
        @media(max-width:480px) { .lapangan-grid { grid-template-columns: 1fr; } }
        .multiselect-dropdown { position: relative; width: 100%; margin-bottom: 18px; }
        .multiselect-header { width: 100%; padding: 12px 14px; border: 1.5px solid var(--border); border-radius: 10px; font-size: 13px; background: #fff; color: var(--text); cursor: pointer; display: flex; align-items: center; justify-content: space-between; user-select: none; transition: all 0.2s; }
        .multiselect-header:hover { border-color: var(--orange); }
        .multiselect-content { position: absolute; top: calc(100% + 4px); left: 0; width: 100%; background: #fff; border: 1.5px solid var(--border); border-radius: 10px; max-height: 200px; overflow-y: auto; z-index: 100; display: none; box-shadow: 0 10px 25px rgba(0, 0, 0, 0.08); padding: 6px 0; }
        .multiselect-content.open { display: block; }
        .multiselect-item { display: flex; align-items: center; gap: 12px; padding: 10px 16px; cursor: pointer; transition: background 0.2s; font-size: 13px; font-weight: 600; color: var(--text-md); margin: 0; }
        .multiselect-item:hover { background: #FAFBFD; }
        .multiselect-item input[type="checkbox"] { cursor: pointer; width: 16px; height: 16px; accent-color: var(--orange); }
        .multiselect-header.error { border-color: var(--red) !important; box-shadow: 0 0 0 3px var(--red-lt) !important; }
        .empty-note-facilities { font-size: 12px; color: var(--muted); font-style: italic; font-weight: 500; }
        .detail-facilities-card { background: #FAFBFD; border: 1px solid var(--border-lt); border-radius: 14px; padding: 14px; margin-bottom: 20px; transition: all .2s ease; text-align: left; }
        .detail-facilities-card:hover { background: #ffffff; border-color: var(--orange); transform: translateY(-1px); box-shadow: 0 4px 12px rgba(0, 0, 0, .02); }
        .detail-facilities-list { display: grid; grid-template-columns: 1fr 1fr; gap: 8px 16px; margin-top: 12px; }
        .facility-item { display: flex; align-items: center; gap: 8px; font-size: 13px; font-weight: 700; color: var(--text); }
        .facility-item i { color: var(--orange); font-size: 11px; }
    </style>
</head>
...
<body>
    <div class="modal-overlay" id="modalLapangan">
        <div class="modal-box">
            <button type="button" class="modal-close" onclick="closeModalDirect('modalLapangan')" title="Tutup"><i class="fa-solid fa-xmark"></i></button>
            <div class="modal-header">
                <div class="modal-subtitle">Kelola Lapangan</div>
                <div class="modal-title" id="formModalTitle">Tambah Lapangan Baru</div>
            </div>
            <div class="modal-body">
                <form method="POST" id="formLapangan" enctype="multipart/form-data" action="lapangan.php" onsubmit="return validateForm()" novalidate>
                    ...
                    <div class="photo-upload-area" id="uploadArea">
                        <input type="file" name="photo_lapangan" id="photo_lapangan" accept="image/jpeg,image/jpg,image/png,image/gif,image/webp" onchange="handlePhotoUpload(this)">
                        <img class="photo-upload-preview" id="previewImg" style="display:none;" alt="Preview">
                        <button type="button" class="photo-upload-remove" id="removeBtn" onclick="event.stopPropagation(); removePhoto();" style="display:none;" title="Hapus Foto"><i class="fa-solid fa-xmark"></i></button>
                    ...
                    <input type="text" name="nama_lapangan" id="nama_lapangan" class="modal-input" placeholder="Contoh: Lapangan Indoor A" autocomplete="off" maxlength="50">
                    <div class="val-msg" id="val-nama_lapangan"></div>
                    <label class="modal-label">Harga Sewa per Jam (Rp) <span class="required">*</span></label>
                    <input type="number" name="harga_sewa" id="harga_sewa" class="modal-input" placeholder="Contoh: 100000" autocomplete="off">
                            ...
                            <?php if (!empty($master_facilities)): ?>
                                <?php foreach ($master_facilities as $fac): ?>
                                    <label class="multiselect-item" for="chk_fac_<?= $fac['ID_Fasilitas'] ?>">
                                        <input type="checkbox" name="use_facility[]" value="<?= $fac['ID_Fasilitas'] ?>" id="chk_fac_<?= $fac['ID_Fasilitas'] ?>" class="facility-checkbox" onchange="updateMultiselectHeader()">
                                        <?= htmlspecialchars($fac['Nama_Fasilitas']) ?>
                                    </label>
                                <?php endforeach; ?>
                            <?php else: ?>
                                <div style="padding: 12px; font-size: 12px; text-align: center; color: var(--muted);">Belum ada master fasilitas aktif terdaftar.</div>
                            <?php endif; ?>
                        </div>
                    </div>
                    <div class="val-msg" id="val-facilities"></div>
                    <button type="submit" class="btn-submit" id="btnSubmitForm"><i class="fa-solid fa-plus"></i> Tambah Lapangan</button>
    ...
    <div class="modal-overlay" id="modalDetail">
        <div class="modal-box detail-modal-box">
            <button type="button" class="modal-close" onclick="closeModalDirect('modalDetail')" title="Tutup"><i class="fa-solid fa-xmark"></i></button>
            ...
            <div class="modal-body" style="padding-top:10px;">
                <div class="detail-photo-wrap">
                    ...
                    <div class="detail-photo-placeholder" id="det_photo_placeholder"><i class="fa-solid fa-layer-group"></i></div>
                </div>
                <div style="text-align: center;">
                    <div class="detail-status-badge" id="det_status_badge"><i class="fa-solid fa-circle"></i><span id="det_status_text">Lapangan Aktif</span></div>
                </div>
                <div class="detail-name" id="det_nama_title">-</div>
                <div class="detail-price" id="det_harga">- <span style="font-size:14px;color:var(--muted);font-family:'Barlow';font-weight:600;">/ jam</span></div>
                <div class="detail-info-grid">
                    ...
                    <div class="detail-info-item">
                        ...
                        <div class="detail-info-value" id="det_tarif_secondary">- <span style="font-size:11px; font-weight:500; color:var(--muted);">/jam</span></div>
                    </div>
                </div>
                ...
                <button type="button" onclick="closeModalDirect('modalDetail')" class="btn-submit" style="background:#0D1117; margin-top: 10px;"><i class="fa-solid fa-arrow-left"></i> Kembali</button>
            </div>
        </div>
    </div>
    ...
    <main class="main">
        ...
        <div class="content">
            <div class="page-header">
                <div><div class="page-title-tag"></div><div class="page-title">Kelola Lapangan</div></div>
                <div class="stat-chips">
                    ...
                    <div class="stat-chip chip-blue"><i class="fa-solid fa-layer-group"></i> TOTAL <span class="chip-val"><?= $total_semua_lapangan ?></span></div>
                </div>
            </div>
            <div class="action-bar">
                <div class="search-box">
                    <i class="fa-solid fa-magnifying-glass"></i>
                    <input type="text" id="src" placeholder="Cari lapangan... (Tekan Enter)" onkeypress="handleSearch(event)" value="<?= htmlspecialchars($_GET['src'] ?? '') ?>">
                    <?php if (!empty($search)): ?>
                ...
                <div style="display:flex;gap:12px;align-items:center;">
                    ...
                    <button type="button" onclick="showAddForm()" class="btn-add"><i class="fa-solid fa-plus"></i>Tambah</button>
                </div>
            </div>
                        ...
                        <div class="lapangan-card" data-name="<?= strtolower(htmlspecialchars($row['Nama_Lapangan'])) ?>">
                            <div class="lapangan-card-photo-wrap" onclick="showDetail('<?= intval($row['ID_Lapangan']) ?>')">
                                ...
                                <?php if (!empty($photo_url)): ?>
                                    <img src="<?= htmlspecialchars($photo_url) ?>" alt="<?= htmlspecialchars($row['Nama_Lapangan']) ?>" loading="lazy" onerror="this.style.display='none';">
                                <?php endif; ?>
                                <span class="lapangan-card-badge <?= $is_aktif ? 'badge-aktif' : 'badge-nonaktif' ?>"><span class="badge-dot"></span> <?= $is_aktif ? 'AKTIF' : 'MAINTENANCE' ?></span>
                                <div class="lapangan-card-actions">
                                    <button type="button" onclick="event.stopPropagation(); showDetail('<?= intval($row['ID_Lapangan']) ?>')" class="lapangan-card-action-btn ac-btn-view" title="Lihat Detail"><i class="fa-solid fa-eye"></i></button>
                                    <button type="button" onclick="event.stopPropagation(); showEditForm('<?= intval($row['ID_Lapangan']) ?>')" class="lapangan-card-action-btn ac-btn-edit" title="Edit Lapangan"><i class="fa-solid fa-pen-to-square"></i></button>
                                    <button type="button" onclick="event.stopPropagation(); doDelete(<?= intval($row['ID_Lapangan']) ?>, '<?= htmlspecialchars($row['Nama_Lapangan'], ENT_QUOTES) ?>')" class="lapangan-card-action-btn ac-btn-delete" title="Hapus Lapangan"><i class="fa-solid fa-trash-can"></i></button>
                                </div>
                            </div>
                            <div class="lapangan-card-info">
                                ...
                                <div class="lapangan-card-meta">
                                    <span class="lapangan-card-harga"><i class="fa-solid fa-money-bill-wave"></i><?= rupiah($row['Harga_Sewa']) ?></span>
                                    <div class="lapangan-card-toggle">
                                        ...
                                        <label class="toggle-switch" title="<?= $is_aktif ? 'Nonaktifkan' : 'Aktifkan' ?>">
                                            <input type="checkbox" <?= $is_aktif ? 'checked' : '' ?> onchange="confirmToggle('<?= intval($row['ID_Lapangan']) ?>', '<?= htmlspecialchars($row['Nama_Lapangan'], ENT_QUOTES) ?>', <?= intval($row['Status']) ?>, event)">
            ...
            <div class="pagination-wrap">
                <div class="pagination-info">
                    <?php if ($total_lapangan > 0): ?>
                        Menampilkan <strong><?= (($page - 1) * $limit) + 1 ?></strong> - <strong><?= min($page * $limit, $total_lapangan) ?></strong> dari <strong><?= $total_lapangan ?></strong> data
                    <?php else: ?>
                ...
                <div class="pagination-nav">
                    ...
                    <a href="?page=<?= max(1, $page - 1) ?><?= $filter_url ?>" class="page-btn <?= $page <= 1 ? 'disabled' : '' ?>"><i class="fa-solid fa-angle-left"></i></a>
                    <?php for ($i = max(1, $page - 2); $i <= min($total_pages, $page + 2); $i++): ?>
                        <a href="?page=<?= $i ?><?= $filter_url ?>" class="page-btn <?= $i == $page ? 'active' : '' ?>"><?= $i ?></a>
                    <?php endfor; ?>
                    <a href="?page=<?= min($total_pages, $page + 1) ?><?= $filter_url ?>" class="page-btn <?= $page >= $total_pages ? 'disabled' : '' ?>"><i class="fa-solid fa-angle-right"></i></a>
...
<script>
        ...
        function validateForm() {
            ...
            if (nama && valNama) {
                ...
                if (errNama) { nama.classList.add('error'); valNama.innerHTML = '<i class="fa-solid fa-circle-exclamation"></i> ' + errNama; valNama.classList.add('show'); valid = false; }
            }
            ...
            if (harga && valHarga) {
                ...
                if (errHarga) { harga.classList.add('error'); valHarga.innerHTML = '<i class="fa-solid fa-circle-exclamation"></i> ' + errHarga; valHarga.classList.add('show'); valid = false; }
            }
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (namaLap) {
                namaLap.addEventListener('input', function () {
                    ...
                    if (v.length > 0 && v.length < 3) { this.classList.add('error'); valNama.innerHTML = '<i class="fa-solid fa-circle-exclamation"></i> Nama lapangan minimal 3 karakter.'; valNama.classList.add('show'); }
                    else if (v.length > 50) { this.classList.add('error'); valNama.innerHTML = '<i class="fa-solid fa-circle-exclamation"></i> Nama lapangan maksimal 50 karakter.'; valNama.classList.add('show'); }
                });
            }
            ...
            if (hargaEl) {
                hargaEl.addEventListener('input', function () {
                    ...
                    if (v !== '' && !isNaN(v)) {
                        if (parseFloat(v) < 10000) { this.classList.add('error'); valHarga.innerHTML = '<i class="fa-solid fa-circle-exclamation"></i> Harga sewa minimal Rp 10.000.'; valHarga.classList.add('show'); }
                        else if (parseFloat(v) > 10000000) { this.classList.add('error'); valHarga.innerHTML = '<i class="fa-solid fa-circle-exclamation"></i> Harga sewa maksimal Rp 10.000.000.'; valHarga.classList.add('show'); }
                    }
                });
            }
        });
        ...
        function confirmToggle(id, name, currentStatus, event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    Swal.fire({ title: 'Memproses...', text: 'Mengubah status lapangan', allowOutsideClick: false, didOpen: function () { Swal.showLoading(); } });
                    setTimeout(function () { window.location.href = '?toggle_id=' + id + '&s=' + currentStatus; }, 600);
                } else {
        ...
        function doDelete(id, name) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    setTimeout(function () { window.location.href = 'lapangan.php?delete_id=' + id; }, 500);
                }
            });
        }

        function resetFilter() { window.location.href = 'lapangan.php'; }
            ...
            fetch('?ajax_detail_id=' + id)
                ...
                .then(res => {
                    if (res.status === 'success') {
                        ...
                        document.getElementById('hiddenInputsArea').innerHTML = '<input type="hidden" name="edit_mode" value="1"><input type="hidden" name="id_lap" value="' + data.ID_Lapangan + '"><input type="hidden" name="edit_photo_path" value="' + (data.Photo_Lapangan ? data.Photo_Lapangan : '') + '">';
            ...
            fetch('?ajax_detail_id=' + id)
                ...
                .then(res => {
                    if (res.status === 'success') {
                        ...
                        } else { detImg.src = ''; detImg.style.display = 'none'; detPlaceholder.style.display = 'flex'; }

                        const badge = document.getElementById('det_status_badge');
                        const text = document.getElementById('det_status_text');
                        if (data.Status == 1) { badge.className = 'detail-status-badge badge-status-aktif'; text.innerText = 'Lapangan Aktif'; }
                        else { badge.className = 'detail-status-badge badge-status-nonaktif'; text.innerText = 'Lapangan Maintenance'; }
                        ...
                        if (detFacList) {
                            if (data.Fasilitas && data.Fasilitas.length > 0) {
                                ...
                                data.Fasilitas.forEach(fac => { facHtml += '<div class="facility-item"><i class="fa-solid fa-circle-check"></i><span>' + fac.Nama_Fasilitas + '</span></div>'; });
                                facHtml += '</div>';
                                detFacList.innerHTML = facHtml;
                            } else { detFacList.innerHTML = '<span class="empty-note-facilities">Tidak ada fasilitas terpasang</span>'; }
                        }
        ...
        function closeModalDirect(modalId) { document.getElementById(modalId).classList.remove('open'); }
        ...
        function updateMultiselectHeader() {
            ...
            if (label) { label.innerText = selectedCount === 0 ? 'Pilih Fasilitas Lapangan' : selectedCount + ' Fasilitas Terpilih'; }
            if (selectedCount > 0 && valFacilities && dropdownHeader) { dropdownHeader.classList.remove('error'); valFacilities.innerHTML = ''; valFacilities.classList.remove('show'); }
        }

        document.addEventListener('click', function (e) {
            const dropdown = document.getElementById('facilityDropdown');
            const content = document.getElementById('multiselectContent');
            if (dropdown && content && !dropdown.contains(e.target)) content.classList.remove('open');
        });
        ...
        function handleSearch(event) {
            if (event.key === 'Enter') {
                ...
                if (keyword) urlParams.set('src', keyword); else urlParams.delete('src');
