...
if (!empty($id_customer)) {
    $st = sqlsrv_query($conn, "SELECT * FROM Customer WHERE ID_Customer=? AND Is_Deleted=0", array($id_customer));
    if ($st) $customer_data = sqlsrv_fetch_array($st, SQLSRV_FETCH_ASSOC);
}
if (!$customer_data) { header("Location: ../login/login.php?status=error&msg=Sesi tidak valid"); exit(); }
...
if ($mc) $member_aktif = sqlsrv_fetch_array($mc, SQLSRV_FETCH_ASSOC);
$has_member  = !empty($member_aktif);
$member_tipe = $has_member ? $member_aktif['Nama_Tipe'] : '';

$tipe_member_list = [];
$qt = sqlsrv_query($conn, "SELECT ID_Tipe,Nama_Tipe,Harga_Member,Potongan_Harga,Status FROM Tipe_Member WHERE Status=1 AND Is_Deleted=0 ORDER BY Harga_Member ASC");
if ($qt) while ($r=sqlsrv_fetch_array($qt,SQLSRV_FETCH_ASSOC)) $tipe_member_list[]=$r;
...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['beli_langganan'])) {
    ...
    if(empty($id_tipe)||empty($metode)){header("Location: langganan_customer.php?status=error&msg=".urlencode("Pilih tipe member dan metode pembayaran!"));exit();}
    $st_tipe=sqlsrv_query($conn,"SELECT * FROM Tipe_Member WHERE ID_Tipe=? AND Status=1 AND Is_Deleted=0",array($id_tipe));
    $tipe_data=sqlsrv_fetch_array($st_tipe,SQLSRV_FETCH_ASSOC);
    if(!$tipe_data){header("Location: langganan_customer.php?status=error&msg=".urlencode("Tipe member tidak valid!"));exit();}
    $cek_aktif=sqlsrv_query($conn,"SELECT COUNT(*) as total FROM Langganan WHERE ID_Customer=? AND Status=1 AND GETDATE() BETWEEN Tanggal_Mulai AND Tanggal_Selesai",array($id_customer));
    $ra=sqlsrv_fetch_array($cek_aktif,SQLSRV_FETCH_ASSOC);
    if($ra['total']>0){header("Location: langganan_customer.php?status=error&msg=".urlencode("Anda masih memiliki langganan aktif."));exit();}
    $cek_pending=sqlsrv_query($conn,"SELECT COUNT(*) as total FROM Langganan WHERE ID_Customer=? AND Status=0",array($id_customer));
    $rp=sqlsrv_fetch_array($cek_pending,SQLSRV_FETCH_ASSOC);
    if($rp['total']>0){header("Location: langganan_customer.php?status=error&msg=".urlencode("Anda memiliki pendaftaran yang sedang menunggu konfirmasi."));exit();}
    $tmulai=date('Y-m-d');$tselesai=date('Y-m-d',strtotime('+30 days'));
    $total=$tipe_data['Harga_Member'];
    $ins=sqlsrv_query($conn,
        "INSERT INTO Langganan(ID_Customer,ID_Karyawan,ID_Tipe,Tanggal_Mulai,Tanggal_Selesai,Total_Bayar,Metode_Pembayaran,Status,Created_By,Created_Date) OUTPUT INSERTED.ID_Langganan VALUES(?,?,?,?,?,?,?,0,?,GETDATE())",
        array($id_customer,2,$id_tipe,$tmulai,$tselesai,$total,$metode,$nama_customer));
    if($ins){
        header("Location: langganan_customer.php?status=success&msg=".urlencode("Pembelian member berhasil! Silakan tunggu konfirmasi dari admin."));exit();
    } else {
        $err=sqlsrv_errors();
        header("Location: langganan_customer.php?status=error&msg=".urlencode("Gagal mendaftar member. ".($err[0]['message']??'')));exit();
    }
}
?>
<!DOCTYPE html>
...
:root {
    --primary:#FF5200;--primary-hover:#E04800;--dark-bg:#0B0B0C;--card-dark:#121214;
    --text-gray:#8E8E93;--white:#FFFFFF;--light-bg:#F8F9FA;
    ...
    --purple:#AF52DE;--purple-lt:rgba(175,82,222,.10);
...
.reveal-stagger .stagger-item{opacity:0;transform:translateY(30px);transition:all .6s cubic-bezier(.16,1,.3,1)}
...
/* ═══ SCROLL PROGRESS ═══ */
.scroll-progress{position:fixed;top:0;left:0;height:3px;background:linear-gradient(90deg,var(--primary),#FF8C42);z-index:9999;transform-origin:left;transform:scaleX(0);transition:transform .1s ease-out}
...
.hero-badge{display:inline-flex;align-items:center;gap:8px;background:var(--primary);color:var(--white);padding:8px 16px;border-radius:50px;font-size:13px;font-weight:700;margin-bottom:20px;animation:fadeInDown .6s cubic-bezier(.16,1,.3,1) .1s forwards;opacity:0}
.hero-badge i{animation:float 2s ease-in-out infinite}
.hero-title{font-size:42px;font-weight:800;color:var(--white);line-height:1.2;margin-bottom:16px;animation:fadeInUp .8s cubic-bezier(.16,1,.3,1) .2s forwards;opacity:0}
.hero-title span{color:var(--primary);display:inline-block;position:relative}
.hero-title span::after{content:'';position:absolute;bottom:5px;left:0;width:0;height:6px;background:rgba(255,82,0,.3);border-radius:3px;z-index:-1;animation:drawLine .8s ease-out .8s forwards}
.hero-desc{color:#A0A0A5;font-size:16px;line-height:1.6;margin-bottom:24px;animation:fadeInUp .8s cubic-bezier(.16,1,.3,1) .4s forwards;opacity:0}
.member-status-card{background:var(--white);border-radius:16px;padding:28px;border:1px solid #E5E5EA;position:relative;z-index:1;min-width:340px;animation:slideInUp .9s cubic-bezier(.16,1,.3,1) .3s forwards;opacity:0;transition:var(--transition-smooth)}
.member-status-card:hover{transform:translateY(-5px);box-shadow:0 20px 40px rgba(0,0,0,.1)}
...
.member-status-icon{width:56px;height:56px;border-radius:16px;display:flex;align-items:center;justify-content:center;font-size:24px;transition:all .4s cubic-bezier(.34,1.56,.64,1)}
...
.member-detail-row{display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid #F2F2F7;transition:var(--transition-smooth)}
.member-detail-row:hover{background:rgba(255,82,0,.03);padding-left:8px;padding-right:8px;border-radius:6px;transform:translateX(5px)}
...
@keyframes slideInUp{from{opacity:0;transform:translateY(60px) scale(.95)}to{opacity:1;transform:translateY(0) scale(1)}}
...
.section-title::after{content:'';position:absolute;bottom:-4px;left:0;width:40px;height:3px;background:var(--primary);border-radius:2px;transition:width .4s cubic-bezier(.16,1,.3,1)}
...
.pricing-card{background:var(--white);border:2px solid #E5E5EA;border-radius:16px;padding:32px;position:relative;transition:all .4s cubic-bezier(.16,1,.3,1)}
.pricing-card:hover{transform:translateY(-8px);box-shadow:0 20px 40px rgba(0,0,0,.12);border-color:rgba(255,82,0,.2)}
...
.pricing-icon{width:56px;height:56px;border-radius:16px;display:flex;align-items:center;justify-content:center;font-size:24px;margin-bottom:20px;transition:all .4s cubic-bezier(.34,1.56,.64,1)}
.pricing-card:hover .pricing-icon{transform:scale(1.1) rotate(5deg)}
.pricing-icon.silver{background:var(--blue-lt);color:var(--blue)}
.pricing-icon.gold{background:rgba(255,149,0,.10);color:#FF9500}
.pricing-icon.platinum{background:var(--purple-lt);color:var(--purple)}
.pricing-name{font-size:22px;font-weight:800;color:#1C1C1E;margin-bottom:4px;transition:color .3s ease}
.pricing-card:hover .pricing-name{color:var(--primary)}
.pricing-desc{font-size:13px;color:#8E8E93;margin-bottom:20px}
.pricing-price{font-size:36px;font-weight:800;color:var(--primary);margin-bottom:4px;transition:all .3s ease}
...
.pricing-features li{display:flex;align-items:center;gap:10px;padding:10px 0;font-size:14px;color:#1C1C1E;border-bottom:1px solid #F2F2F7;transition:all .3s ease}
.pricing-features li:hover{transform:translateX(5px)}
.pricing-features li:last-child{border-bottom:none}
.pricing-features li i{color:var(--green);font-size:14px;transition:transform .3s ease}
.pricing-features li:hover i{transform:scale(1.2)}
.btn-pilih{width:100%;background:var(--primary);color:var(--white);border:none;padding:14px;border-radius:10px;font-size:14px;font-weight:700;cursor:pointer;transition:all .3s cubic-bezier(.16,1,.3,1);display:flex;align-items:center;justify-content:center;gap:8px;position:relative;overflow:hidden;font-family:inherit}
.btn-pilih::before{content:'';position:absolute;top:50%;left:50%;width:0;height:0;background:rgba(255,255,255,.2);border-radius:50%;transform:translate(-50%,-50%);transition:width .6s,height .6s}
.btn-pilih:hover::before{width:300px;height:300px}
.btn-pilih:hover{background:var(--primary-hover);transform:translateY(-2px);box-shadow:0 8px 25px rgba(255,82,0,.4)}
.btn-pilih:disabled{background:#C7C7CC;cursor:not-allowed;transform:none;box-shadow:none}

/* ═══ MODAL PEMBAYARAN ═══ */
.modal-overlay{display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(15,23,42,.6);z-index:2000;align-items:center;justify-content:center;backdrop-filter:blur(4px);animation:fadeInModal .25s ease-out forwards}
.modal-overlay.active{display:flex}
.modal-box{background:var(--white);border-radius:20px;width:480px;max-width:90%;max-height:90vh;overflow-y:auto;padding:32px;animation:slideInModal .3s cubic-bezier(.16,1,.3,1) forwards;position:relative;-ms-overflow-style:none;scrollbar-width:none}
.modal-box::-webkit-scrollbar{display:none}
.modal-header{display:flex;align-items:center;justify-content:space-between;margin-bottom:24px}
.modal-header h3{font-size:20px;font-weight:800;color:#1C1C1E}
.modal-close{width:36px;height:36px;border-radius:50%;background:#F2F2F7;border:none;display:flex;align-items:center;justify-content:center;cursor:pointer;color:#8E8E93;font-size:14px;transition:var(--transition-smooth)}
...
.modal-summary-row{display:flex;justify-content:space-between;align-items:center;padding:8px 0;font-size:14px;transition:var(--transition-smooth)}
...
/* ═══ PAYMENT CARDS (sama dgn booking) ═══ */
...
.btn-bayar{width:100%;background:var(--orange);color:var(--white);border:none;padding:16px;border-radius:12px;font-size:15px;font-weight:800;cursor:pointer;transition:var(--transition-smooth);display:flex;align-items:center;justify-content:center;gap:8px;position:relative;overflow:hidden;margin-top:16px;font-family:inherit}
.btn-bayar::before{content:'';position:absolute;top:50%;left:50%;width:0;height:0;background:rgba(255,255,255,.2);border-radius:50%;transform:translate(-50%,-50%);transition:width .6s,height .6s}
.btn-bayar:hover::before{width:400px;height:400px}
.btn-bayar:hover{background:var(--orange-hover);transform:translateY(-2px);box-shadow:0 10px 30px rgba(255,90,31,.4)}
...
/* ═══ INSTRUKSI PEMBAYARAN (identik dengan booking) ═══ */
.instr-overlay{position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(15,23,42,.6);backdrop-filter:blur(4px);display:none;align-items:center;justify-content:center;z-index:2001;padding:20px}
.instr-overlay.active{display:flex;animation:fadeInModal .25s ease-out forwards}
.instr-card{background:#fff;border-radius:20px;padding:30px;width:100%;max-width:460px;max-height:90vh;overflow-y:auto;position:relative;box-shadow:0 20px 40px rgba(0,0,0,.15);animation:slideInModal .3s cubic-bezier(.16,1,.3,1) forwards;text-align:center;-ms-overflow-style:none;scrollbar-width:none}
.instr-card::-webkit-scrollbar{display:none}
.instr-close{position:absolute;top:20px;right:20px;background:var(--border-lt);border:none;width:32px;height:32px;border-radius:50%;display:flex;align-items:center;justify-content:center;cursor:pointer;color:var(--text-secondary);transition:var(--transition-smooth);z-index:10}
.instr-close:hover{background:var(--red-lt);color:var(--red);transform:rotate(90deg)}
.instr-title{font-family:'Barlow Condensed',sans-serif;font-size:16px;font-weight:800;letter-spacing:.5px;color:var(--muted);margin-bottom:20px;text-transform:uppercase;text-align:center}
...
.total-display-box{background:var(--bg);padding:14px 18px;border-radius:12px;margin-bottom:20px;border:1px solid var(--border)}
...
.steps-label{font-size:11.5px;font-weight:700;color:var(--text-secondary);margin-bottom:14px;text-transform:uppercase;letter-spacing:.5px;margin-top:20px;text-align:left}
.step-item{display:flex;gap:14px;align-items:flex-start;padding:14px 16px;background:#fafafa;border-radius:12px;border:1px solid var(--border-lt);transition:var(--transition-smooth);margin-bottom:12px;text-align:left}
...
.qris-wrap{display:none;align-items:center;flex-direction:column}
.qris-wrap.active{display:flex}
.qris-title-label{font-size:12.5px;font-weight:700;color:var(--text-primary);margin-bottom:12px}
...
.footer-col ul li a{color:#8E8E93;text-decoration:none;font-size:13px;transition:all .3s ease;display:inline-block}
...
@media(max-width:1100px){.pricing-grid{grid-template-columns:1fr}.hero{flex-direction:column;padding:40px}.member-status-card{min-width:auto;width:100%}.main-container{padding:40px}nav{padding:0 40px}.footer-grid{grid-template-columns:repeat(2,1fr)}}
@media(max-width:768px){.nav-links{display:none}.main-container{padding:20px}nav{padding:0 20px}.hero{padding:30px 20px}.hero-title{font-size:28px}.footer-grid{grid-template-columns:1fr}.payment-grid{grid-template-columns:1fr}}
</style>
</head>
<body>
...
<!-- NAVBAR -->
<nav>
    <a href="view_customer.php" class="nav-logo"><img src="../asset/image/logo2.png" alt="HoopBall"></a>
    <div class="nav-links">
    ...
    <div class="nav-user-container">
        <div class="nav-user">
            <?php if (!empty($photo_profile) && file_exists($photo_profile)): ?><img src="<?= htmlspecialchars($photo_profile) ?>" alt="Avatar" class="user-avatar">
            <?php else: ?><i class="fa-solid fa-circle-user user-icon"></i><?php endif; ?>
            <span><?= htmlspecialchars($nama_customer) ?></span>
            <?php if ($has_member): ?><span class="member-badge-nav"><i class="fa-solid fa-crown"></i> <?= htmlspecialchars($member_tipe) ?></span><?php endif; ?>
...
<section class="hero">
    <div class="hero-left">
        <div class="hero-badge"><i class="fa-solid fa-crown"></i> LANGGANAN MEMBER</div>
    ...
    <div class="member-status-card">
        <div class="member-status-header">
            <div class="member-status-icon <?= $has_member?'active':($member_aktif?'pending':'inactive') ?>">
                <i class="fa-solid <?= $has_member?'fa-crown':($member_aktif?'fa-clock':'fa-user') ?>"></i>
            </div>
            <div class="member-status-text">
                <h3><?= $has_member?'Member '.htmlspecialchars($member_tipe).' Aktif':($member_aktif?'Menunggu Konfirmasi':'Belum Berlangganan') ?></h3>
                <p><?= $has_member?'Nikmati keuntungan member Anda':($member_aktif?'Pendaftaran member sedang diproses admin':'Daftar sekarang untuk mendapatkan keuntungan') ?></p>
            </div>
        </div>
        <?php if ($has_member): ?>
            <div class="member-detail-row"><span class="member-detail-label">Tipe Member</span><span class="member-detail-value primary"><?= htmlspecialchars($member_tipe) ?></span></div>
            <div class="member-detail-row"><span class="member-detail-label">Potongan Harga</span><span class="member-detail-value green"><?= rupiahFormat($member_aktif['Potongan_Harga']) ?> /booking</span></div>
            <div class="member-detail-row"><span class="member-detail-label">Tanggal Mulai</span><span class="member-detail-value"><?= formatTanggal($member_aktif['Tanggal_Mulai']) ?></span></div>
            <div class="member-detail-row"><span class="member-detail-label">Berlaku Sampai</span><span class="member-detail-value"><?= formatTanggal($member_aktif['Tanggal_Selesai']) ?></span></div>
            <div class="member-detail-row"><span class="member-detail-label">Sisa Hari</span><span class="member-detail-value green"><?php
                $ts=($member_aktif['Tanggal_Selesai'] instanceof DateTime)?$member_aktif['Tanggal_Selesai']->getTimestamp():strtotime($member_aktif['Tanggal_Selesai']);
                $sisa=ceil(($ts-time())/86400);echo $sisa>0?$sisa.' hari':'Hari ini berakhir';
            ?></span></div>
        <?php elseif ($member_aktif): ?>
            <div class="member-detail-row"><span class="member-detail-label">Tipe Member</span><span class="member-detail-value primary"><?= htmlspecialchars($member_aktif['Nama_Tipe']) ?></span></div>
            <div class="member-detail-row"><span class="member-detail-label">Total Bayar</span><span class="member-detail-value"><?= rupiahFormat($member_aktif['Total_Bayar']) ?></span></div>
            <div class="member-detail-row"><span class="member-detail-label">Tanggal Daftar</span><span class="member-detail-value"><?= formatTanggal($member_aktif['Created_Date']) ?></span></div>
            <div class="member-detail-row"><span class="member-detail-label">Status</span><span class="member-detail-value yellow">Menunggu Konfirmasi Admin</span></div>
        <?php else: ?>
            <div class="member-detail-row"><span class="member-detail-label">Status</span><span class="member-detail-value" style="color:var(--red)">Non-Member</span></div>
            <div class="member-detail-row"><span class="member-detail-label">Potongan Harga</span><span class="member-detail-value">-</span></div>
            <p style="font-size:13px;color:#8E8E93;margin-top:16px;text-align:center;line-height:1.5">Pilih paket member di bawah untuk mulai berlangganan</p>
        <?php endif; ?>
    </div>
</section>
...
<main class="main-container">
    <section>
        <div class="section-header reveal">
            <div><h2 class="section-title">Pilih Paket Member</h2><p class="section-subtitle">Pilih tipe member yang sesuai dengan kebutuhan Anda.</p></div>
        </div>
        <div class="pricing-grid reveal-stagger">
            <?php
            $icon_map=['Silver'=>'fa-medal','Gold'=>'fa-trophy','Platinum'=>'fa-crown'];
            $cls_map=['Silver'=>'silver','Gold'=>'gold','Platinum'=>'platinum'];
            foreach ($tipe_member_list as $tipe):
                $is_rec=($tipe['Nama_Tipe']==='Gold');
                $icon=$icon_map[$tipe['Nama_Tipe']]??'fa-star';
                $cls=$cls_map[$tipe['Nama_Tipe']]??'silver';
            ?>
            <div class="pricing-card stagger-item <?= $is_rec?'recommended':'' ?>">
                <div class="pricing-icon <?= $cls ?>"><i class="fa-solid <?= $icon ?>"></i></div>
                <h3 class="pricing-name"><?= htmlspecialchars($tipe['Nama_Tipe']) ?></h3>
                <p class="pricing-desc">Paket member <?= htmlspecialchars($tipe['Nama_Tipe']) ?> 30 hari</p>
                <div class="pricing-price"><?= rupiahFormat($tipe['Harga_Member']) ?><span> / 30 hari</span></div>
                <div class="pricing-potongan"><i class="fa-solid fa-tag"></i> Hemat <?= rupiahFormat($tipe['Potongan_Harga']) ?> per booking</div>
                <ul class="pricing-features">
                    <li><i class="fa-solid fa-check"></i> Potongan <?= rupiahFormat($tipe['Potongan_Harga']) ?> per booking</li>
                    ...
                    <?php if ($tipe['Nama_Tipe']==='Platinum'): ?><li><i class="fa-solid fa-check"></i> Diskon pembelian alat 5%</li><?php endif; ?>
                </ul>
                <button class="btn-pilih"
                    onclick="bukaModal(<?= $tipe['ID_Tipe'] ?>,'<?= htmlspecialchars($tipe['Nama_Tipe']) ?>',<?= $tipe['Harga_Member'] ?>)"
                    <?= ($has_member||$member_aktif)?'disabled':'' ?>>
                    <?= $has_member?'Sudah Aktif':($member_aktif?'Menunggu Konfirmasi':'<i class="fa-solid fa-crown"></i> Pilih Paket Ini') ?>
                </button>
            </div>
            <?php endforeach; ?>
        </div>
...
<div class="modal-overlay" id="paymentModal">
    <div class="modal-box">
        ...
        <div class="modal-summary">
            <div class="modal-summary-row"><span>Tipe Member</span><span id="modalTipeName" style="font-weight:700">-</span></div>
            <div class="modal-summary-row"><span>Durasi</span><span>30 hari</span></div>
            <div class="modal-summary-row"><span>Tanggal Mulai</span><span id="modalTanggalMulai">-</span></div>
            <div class="modal-summary-row"><span>Tanggal Selesai</span><span id="modalTanggalSelesai">-</span></div>
            <div class="modal-summary-row total"><span>Total Pembayaran</span><span id="modalTotal">Rp 0</span></div>
        </div>
        ...
        <button type="button" class="btn-bayar" onclick="prosesBayar()"><i class="fa-solid fa-lock"></i> Bayar Sekarang</button>
        <div class="booking-disclaimer"><i class="fa-solid fa-circle-check"></i> Enkripsi data aman terverifikasi</div>
    </div>
</div>

<!-- MODAL INSTRUKSI PEMBAYARAN (identik dengan booking) -->
<div class="instr-overlay" id="instructionModal">
    <div class="instr-card">
        <button class="instr-close" onclick="tutupInstructionModal()"><i class="fa-solid fa-xmark"></i></button>
        <p class="instr-title">Instruksi Pembayaran</p>
        ...
        <div class="countdown-box"><i class="fa-solid fa-clock"></i><p class="countdown-text">Selesaikan pembayaran dalam <span id="paymentCountdown">15:00</span></p></div>
        <div class="total-display-box"><div class="total-display-label">Total Tagihan</div><div class="total-display-amount" id="instructionTotal">Rp 0</div></div>

        <div id="instruksiTransfer">
            <div class="bank-info-card">
                <div class="bank-header"><div class="bank-icon"><i class="fa-solid fa-building-columns"></i></div><div><div class="bank-title">Virtual Account</div><div class="bank-sub">Mandiri / BCA / BNI / BRI</div></div></div>
            ...
            <div class="step-item"><div class="step-num">1</div><div><div class="step-title">Buka Aplikasi Banking</div><div class="step-desc">Pilih menu <strong style="color:var(--primary)">Transfer &gt; Virtual Account</strong>.</div></div></div>
            <div class="step-item"><div class="step-num">2</div><div><div class="step-title">Masukkan Nomor VA</div><div class="step-desc">Masukkan nomor <strong style="color:var(--primary)">8801281234567890</strong>.</div></div></div>
            <div class="step-item" style="margin-bottom:0"><div class="step-num">3</div><div><div class="step-title">Konfirmasi Pembayaran</div><div class="step-desc">Nominal akan otomatis muncul. Konfirmasi dan selesaikan.</div></div></div>
        </div>

        <div id="instruksiQRIS" class="qris-wrap">
            <div class="qris-title-label">Pindai Kode QRIS Resmi HoopBall</div>
            <div class="qris-img-wrap"><img id="qrisImage" src="" alt="QRIS Code" class="qris-img"></div>
            <ul class="qris-steps-list"><li>Buka aplikasi e-wallet (GoPay, OVO, Dana, LinkAja) atau Mobile Banking.</li><li>Pilih opsi <strong>Scan / Bayar QRIS</strong>.</li><li>Arahkan kamera ke kode QR, lalu selesaikan pembayaran.</li></ul>
        </div>

        <hr class="modal-divider">
        <button class="btn-done-pay" id="btnDonePayment" onclick="selesaiBayar()">Saya Sudah Bayar <i class="fa-solid fa-circle-check"></i></button>
    </div>
</div>
...
<footer>
    <div class="footer-grid">
        <div>
            ...
            <div class="social-links"><a href="#" class="social-btn"><i class="fa-brands fa-instagram"></i></a><a href="#" class="social-btn"><i class="fa-brands fa-facebook-f"></i></a><a href="#" class="social-btn"><i class="fa-brands fa-tiktok"></i></a><a href="#" class="social-btn"><i class="fa-brands fa-youtube"></i></a></div>
        </div>
        ...
        <div class="footer-col"><h4>Hubungi Kami</h4><div class="contact-item"><i class="fa-solid fa-location-dot"></i>Jl. Olahraga No. 10, Kebayoran Baru, Jakarta Selatan 12190</div><div class="contact-item"><i class="fa-solid fa-phone"></i>+62 812-3456-7890</div><div class="contact-item"><i class="fa-solid fa-envelope"></i>info@hoopball.id</div><div class="contact-item"><i class="fa-solid fa-clock"></i>Setiap hari 07:00 – 23:00 WIB</div></div>
    </div>
...
<script>
/* ─── SCROLL PROGRESS ─── */
window.addEventListener('scroll',()=>{const st=document.documentElement.scrollTop||document.body.scrollTop;const sh=document.documentElement.scrollHeight-document.documentElement.clientHeight;document.getElementById('scrollProgress').style.transform=`scaleX(${st/sh})`});
...
let selectedIdTipe=0,selectedTipeName='',selectedHarga=0,selectedMetode='Transfer Bank',countdownInterval;

function formatRupiah(n){return'Rp '+n.toLocaleString('id-ID')}
function formatTanggalJS(d){const b=['Jan','Feb','Mar','Apr','Mei','Jun','Jul','Agu','Sep','Okt','Nov','Des'];return d.getDate()+' '+b[d.getMonth()]+' '+d.getFullYear()}

function bukaModal(idTipe,namaTipe,harga){
    selectedIdTipe=idTipe;selectedTipeName=namaTipe;selectedHarga=harga;
    const today=new Date(),selesai=new Date();selesai.setDate(today.getDate()+30);
    document.getElementById('modalTipeName').textContent=namaTipe;
    document.getElementById('modalTanggalMulai').textContent=formatTanggalJS(today);
    document.getElementById('modalTanggalSelesai').textContent=formatTanggalJS(selesai);
...
function tutupModal(){document.getElementById('paymentModal').classList.remove('active');document.body.style.overflow=''}
function pilihMetode(el){document.querySelectorAll('.payment-card').forEach(p=>p.classList.remove('selected'));el.classList.add('selected');selectedMetode=el.getAttribute('data-method')}
function prosesBayar(){tutupModal();document.getElementById('instructionModal').classList.add('active');document.body.style.overflow='hidden';showPaymentMethodInstructions(selectedMetode);startPaymentCountdown(15*60)}
...
function showPaymentMethodInstructions(method){
    ...
    if(method==='Transfer Bank'){btnVA.classList.add('active');btnQR.classList.remove('active');va.style.display='block';qris.classList.remove('active')}
    else{btnQR.classList.add('active');btnVA.classList.remove('active');va.style.display='none';qris.classList.add('active');document.getElementById('qrisImage').src='https://api.qrserver.com/v1/create-qr-code/?size=180x180&data='+encodeURIComponent('HOOPBALL-MEMBER-'+selectedIdTipe+'-'+selectedHarga+'-'+Date.now())}
}
...
document.getElementById('btnCopyVA').addEventListener('click',()=>{
    ...
    navigator.clipboard.writeText(v.value).then(()=>{
        Swal.fire({icon:'success',title:'Berhasil Disalin!',text:'Nomor VA disalin ke papan klip.',toast:true,position:'top-end',timer:2000,showConfirmButton:false,customClass:{popup:'swal-toast'}});
    });
});
...
function selesaiBayar(){
    ...
    fd.append('beli_langganan','1');fd.append('id_tipe',selectedIdTipe);fd.append('metode_pembayaran',selectedMetode);
    fetch('langganan_customer.php',{method:'POST',body:fd})
        .then(()=>{
            Swal.fire({icon:'success',title:'Pembayaran Diterima!',text:'Pembelian member berhasil. Silakan tunggu konfirmasi dari admin.',confirmButtonColor:'var(--orange)',confirmButtonText:'OK'}).then(()=>window.location.reload());
        })
        .catch(()=>window.location.reload());
}

window.addEventListener('click',e=>{
    if(e.target===document.getElementById('paymentModal'))tutupModal();
    if(e.target===document.getElementById('instructionModal'))tutupInstructionModal();
});
...
if(status&&msg){
    ...
    Swal.fire({icon:ok?'success':'error',title:ok?'Berhasil':'Gagal',text:msg,toast:true,position:'top-end',timer:4000,timerProgressBar:true,showConfirmButton:false,showCloseButton:true,background:'#ffffff',color:'#1c1c1e',iconColor:ok?'#34C759':'#FF3B30',customClass:{popup:'swal-toast'}});
