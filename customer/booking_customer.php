    ...
    for ($i = 0; $i < 7; $i++) {
        ...
        foreach ($list as $id_lap) {
            foreach ($slots as $s) {
                $cek = sqlsrv_query(
                    $conn,
                    "SELECT ID_Jadwal FROM Jadwal WHERE ID_Lapangan=? AND Tanggal=? AND Jam_Mulai=? AND Jam_Selesai=?",
                    array($id_lap, $d, $s[0], $s[1])
                );
                if ($cek && !sqlsrv_fetch_array($cek, SQLSRV_FETCH_ASSOC))
                    sqlsrv_query(
                        $conn,
                        "INSERT INTO Jadwal(ID_Lapangan,Tanggal,Jam_Mulai,Jam_Selesai,Status,Is_Deleted,Created_By,Created_Date) VALUES(?,?,?,?,1,0,'SYSTEM_AUTO',GETDATE())",
                        array($id_lap, $d, $s[0], $s[1])
                    );
            }
        }
...
if (isset($_GET['action'])) {
    ...
    if ($_GET['action'] == 'checkout' && $_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if ($id_jadwal <= 0 || empty($metode) || $total <= 0) {
        ...
        try {
            ...
            $jadwal = null;
            if ($chk)
                $jadwal = sqlsrv_fetch_array($chk, SQLSRV_FETCH_ASSOC);
...
<head>
    ...
    <link
        href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@300;400;500;600;700;800&family=Barlow+Condensed:wght@700;800;900&display=swap"
        rel="stylesheet">
...
/* ═══ PAGE CONTAINER ═══ */
        ...
        .booking-modal-overlay {
            ...
            z-index: 2000;
            padding: 20px;
            animation: fadeInModal .25s ease-out forwards
        }
        ...
        .summary-card {
            ...
            animation: slideInModal .3s cubic-bezier(.16, 1, .3, 1) forwards;
            -ms-overflow-style: none;
            scrollbar-width: none
        }
        ...
        .btn-trigger-modal:hover::before {
            width: 300px;
            height: 300px
        }
        ...
        .btn-trigger-modal:hover:not(:disabled) {
            ...
            box-shadow: 0 8px 25px rgba(255, 90, 31, .4)
        }
        ...
        .bank-icon {
            ...
            color: #fff;
            font-size: 18px;
            box-shadow: 0 4px 12px rgba(255, 82, 0, .3)
        }
        ...
        .btn-copy-va:hover {
            transform: translateY(-2px);
            box-shadow: 0 6px 20px rgba(255, 82, 0, .4)
        }
        ...
        .social-btn:hover {
            ...
            box-shadow: 0 8px 20px rgba(255, 82, 0, .3)
        }
...
<body>
    ...
    <!-- NAVBAR (shared, sama persis dengan Beranda) -->
    <?php $path_prefix = '../'; 
    ...
    <div class="container">
        <div class="section-header reveal">
            <h2 class="section-title"><i class="fa-solid fa-basketball" style="color:var(--primary)"></i> Pilih Lapangan
            </h2>
        ...
        <div class="court-grid reveal-stagger">
            <?php if (!empty($lapanganList)): ?>
                <?php foreach ($lapanganList as $idx => $lap):
                    ...
                    ?>
                    <div class="court-card stagger-item <?= $sel ?>" data-id="<?= $cId ?>" data-price="<?= $cPrice ?>"
                            ...
                            <ul class="court-perk-list">
                                <?php if (isset($lapanganFasilitas[$cId])): ?>
                                    <?php foreach ($lapanganFasilitas[$cId] as $f): ?>
                                        <li class="court-perk-item"><i class="fa-solid fa-circle-check"></i> <?= htmlspecialchars($f) ?>
                                        </li>
                                    <?php endforeach; ?>
                                <?php else: ?>
                ...
                <?php endforeach; ?>
            <?php else: ?>
                <p style="grid-column:span 3;text-align:center;color:var(--muted);padding:40px">Tidak ada lapangan aktif
                    saat ini.</p>
            <?php endif; ?>
        </div>
    </div>
    ...
    <div class="booking-modal-overlay" id="scheduleModal">
        <div class="summary-card" style="max-width:600px">
            ...
            <div class="section-header">
                <h2 class="section-title"><i class="fa-solid fa-calendar-days" style="color:var(--primary)"></i> Pilih
                    Jadwal Bermain</h2>
            ...
            <div class="alert-banner">
                ...
                <p class="alert-banner-text">Semua slot jadwal disesuaikan dengan daftar yang dibuat oleh operator
                    HoopBall Arena.</p>
            </div>
        ...
        <div class="summary-card">
            ...
            <div class="member-block">
                <div class="member-status-header">
                    ...
                    <?php if ($has_member): ?>
                        <span class="badge-member-active">Member <?= htmlspecialchars($member_tipe) ?> <i
                ...
                <?php if ($has_member): ?>
                    ...
                    <div class="discount-row">
                        ...
                        <span class="discount-val" id="lblDiscountPercent">-Rp
                            <?= number_format($member_discount, 0, ',', '.') ?></span>
                    </div>
                <?php else: ?>
            ...
            <div class="pricing-breakdown">
                <div class="price-row"><span id="lblNormalPriceLabel">Harga Sewa</span><span id="lblNormalPrice">Rp
                        0</span></div>
                <?php if ($has_member): ?>
                    <div class="price-row"><span>Potongan Member</span><span class="discount-val"
                            id="lblDiscountBreakdown">-Rp <?= number_format($member_discount, 0, ',', '.') ?></span></div>
                <?php else: ?>
                    <div class="price-row"><span>Potongan Promo</span><span class="discount-val" id="lblPromoBreakdown">-Rp
                            0</span></div>
                <?php endif; ?>
                <div class="price-row total-row"><span>Total Pembayaran</span><span class="total-amount"
                        id="lblTotalPrice">Rp 0</span></div>
            </div>
            ...
            <div class="payment-section">
                ...
                <div class="payment-grid">
                    <div class="payment-card selected" data-method="Transfer Bank">
                        ...
                        <div class="payment-card-content"><span class="payment-name">Transfer Bank</span><span
                                class="payment-sub">Virtual Account</span></div>
                    </div>
                    <div class="payment-card" data-method="QRIS">
                        <div class="custom-radio"></div>
                        <div class="payment-card-content"><span class="payment-name qris-logo">QRIS</span><span
                                class="payment-sub">Scan &amp; Bayar Instan</span></div>
                    </div>
                </div>
            </div>
            <button class="btn-booking" id="btnSubmit" disabled><i class="fa-solid fa-lock"></i> Selesaikan
                Booking</button>
            <div class="booking-disclaimer"><i class="fa-solid fa-circle-check"></i> Enkripsi data aman terverifikasi
            </div>
        </div>
    </div>
    ...
    <div class="booking-modal-overlay" id="paymentInstructionModal">
        <div class="summary-card" style="max-width:460px;text-align:center">
            <button class="booking-modal-close" onclick="tutupInstructionModal()"><i
                    class="fa-solid fa-xmark"></i></button>
            <p class="instruction-title">Instruksi Pembayaran</p>
            <div class="switch-tabs-row">
                <button id="btnSwitchVA" class="switch-tab-btn active"
                    onclick="showPaymentMethodInstructions('Transfer Bank')">
            ...
            <div id="instruksiTransfer" class="instr-va-box">
                <div class="bank-info-card">
                    ...
                    <div class="va-input-row">
                        <div class="va-input-wrap"><i class="fa-solid fa-hashtag"></i><input type="text" id="vaNumber"
                                value="8801281234567890" readonly></div>
                ...
                <div class="step-item">
                    ...
                    <div>
                        ...
                        <div class="step-desc">Pilih menu <strong style="color:var(--primary)">Transfer &gt; Virtual
                                Account</strong> pada M-Banking atau ATM Anda.</div>
                    </div>
                </div>
                <div class="step-item">
                    ...
                    <div>
                        ...
                        <div class="step-desc">Masukkan nomor Virtual Account <strong
                                style="color:var(--primary)">8801281234567890</strong>.</div>
                    </div>
                </div>
                <div class="step-item" style="margin-bottom:0">
                    ...
                    <div>
                        ...
                        <div class="step-desc">Nominal akan otomatis muncul sesuai total tagihan. Konfirmasi dan
                            selesaikan.</div>
                    </div>
                </div>
            </div>
            ...
            <button class="btn-done-pay" id="btnDonePayment">Saya Sudah Bayar <i
    ...
    <footer>
        <div class="footer-grid">
            <div>
                ...
                <p class="footer-desc">HoopBall adalah platform penyewaan lapangan basket online yang mudah, cepat, dan
                    terpercaya.</p>
            ...
            <div class="footer-col">
                <h4>Hubungi Kami</h4>
                <div class="contact-item"><i class="fa-solid fa-location-dot"></i>Jl. Olahraga No. 10, Kebayoran Baru,
