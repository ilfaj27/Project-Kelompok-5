...
if (isset($_GET['toggle_id']) && isset($_GET['s'])) {
    ...
    $s_baru = ($_GET['s'] == 1) ? 0 : 1;
    $stmt = sqlsrv_query($conn, "UPDATE Jadwal SET Status = ? WHERE ID_Jadwal = ?", array($s_baru, $id_toggle));
    header("Location: index.php?status=success&msg=Status jadwal berhasil diubah!"); 
    exit();
}
...
// Filter Status (Tersedia = 1, Booked/Tidak Tersedia = 0)
...
$tot_jadwal = $q_tot ? (sqlsrv_fetch_array($q_tot, SQLSRV_FETCH_ASSOC)['t'] ?? 0) : 0;

$q_aktif = sqlsrv_query($conn, "SELECT COUNT(*) as t FROM Jadwal WHERE Is_Deleted = 0 AND Status = 1");
$tot_aktif = $q_aktif ? (sqlsrv_fetch_array($q_aktif, SQLSRV_FETCH_ASSOC)['t'] ?? 0) : 0;
        ...
        <div class="content">
            ...
            <div class="action-bar">
                ...
                <div style="display: flex; gap: 12px;">
                    ...
                    <a href="action/create.php" class="btn-add"><i class="fa-solid fa-plus"></i> Buat Jadwal</a>
                </div>
            </div>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <div class="time-text" style="margin-top:2px; color:var(--blue);"><i class="fa-regular fa-clock"></i> <?= $jm ?> - <?= $js ?></div>
                            </td>
                            <td>
                                <span class="status-badge <?= $is_aktif ? 'sb-aktif' : 'sb-habis' ?>">
                                    <span class="status-dot"></span> <?= $is_aktif ? 'Tersedia' : 'Tidak Tersedia' ?>
                                </span>
                            </td>
                            <td>
                                <div class="actions">
                                    <a href="?detail_id=<?= $row['ID_Jadwal'] ?>" class="btn-action btn-view" title="Detail"><i class="fa-solid fa-eye"></i></a>
                                    <a href="action/update.php?id=<?= $row['ID_Jadwal'] ?>" class="btn-action btn-edit" title="Edit"><i class="fa-solid fa-pen-to-square"></i></a>
            ...
            <div class="modal-body">
                ...
                ?>
                    ...
                    <div class="info-row">
                        ...
                        <span class="info-val time-text" style="font-size:20px; color:var(--blue);"><?= $jm ?> - <?= $js ?></span>
                    </div>
...
function confirmToggle(id, status) {
    ...
    Swal.fire({
        title: 'Konfirmasi', text: `Yakin ingin ${action} jadwal ini?`, icon: 'warning',
...
function confirmDelete(id, name) {
    Swal.fire({
        title: 'Hapus Jadwal?', html: `Hapus jadwal <strong style="color:red">${name}</strong> secara permanen?`, icon: 'error',
